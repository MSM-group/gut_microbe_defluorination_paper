...
p2 <- ggplot(specdf, aes(x = position_index, y = value)) +
  # scale_y_log10(aes(value)) +
  geom_violin(aes(color = as.factor(chimera_segment), fill = as.factor(chimera_segment),alpha = 0.95)) +
  geom_point(aes(color = as.factor(chimera_segment), shape = as.factor(variable), fill = as.factor(chimera_segment),alpha = 0.95)) +
  theme_pubr() +
...
summary(specdf$value)
specdf$value[specdf$label == "WT"]
# Make a segment plot
p1 <-  ggplot(specdf, aes(x = position_index, y = log10(value))) +
 # scale_y_log10(aes(value)) +
  ...
  xlab("Protein position") +
  ylab("Enzyme activity \n normalized by wild-type") +
  geom_line(aes(y=log10(830.7176)), color = "gray20", linetype = "dashed", linewidth = 0.5)
p1  
ggsave("output/alanine_variant_mean_points.png", p1)
...
ggsave("output/alanine_variant_mean_pos_points_violin.png", p2)
